...
`.nudge/**/*.yaml`. Rules describe deterministic or opt-in semantic conventions. Learned incident notes
...
- `action` defaults to `block`. Other supported actions are `substitute` and `warn` (semantic rules only).
