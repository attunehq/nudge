...
Pavlov uses Claude Code's [hooks system](https://docs.anthropic.com/en/docs/claude-code/hooks) to watch what it does. When something matches a rule you've defined:
...
These are the rules Pavlov uses on its own codebase (yes, we dogfood):
...
Attune dogfoods Pavlov on its own codebase and on other codebases we manage. When we notice Claude routing around a rule or missing the point, we tune the message until it lands. Treat ignored rules as feedback on clarity, not evidence that rules don't work.
...
> [!NOTE]
> Claude Code loads hooks on startup, so you'll need to restart open sessions (`claude -c` is an easy way to do this without breaking your flow). Future changes to rules are internal to Pavlov and therefore do not need a Claude Code restart.

### 3. Use Claude Code Normally

Pavlov runs automatically as you use Claude Code. No changes to your workflow required.

## Seeing It Work

### In Practice

Write some rules for things that you want to be enforced, and then just use Claude Code normally. You should see Pavlov interject when the rules are violated and help Claude stay on track.
...
# Exit code 2 = Interrupt (Pavlov caught something, instructs Claude to reconsider)
